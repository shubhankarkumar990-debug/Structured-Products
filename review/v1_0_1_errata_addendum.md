# V1.0.1 ERRATA ADDENDUM

**Package:** Desk Bible Ecosystem V1.0.1 Semantic Errata  
**Effective:** 2026-06-27  
**Scope:** 7 confirmed errata items across 3 frozen artifacts  
**Canonical convention:** See `correlation_convention_framework.md`  
**Status:** FINAL  
...
**V1.0.1 corrected text:**
> | FTD (N=1) | 9.5% | Highest | Long correlation (MTM: benefits from high ρ) |
...
**Error:** 65% is not above 100%. The statement is mathematically false. Under the standard WOAC structure where strike = 100% of initial, ASML at 65% is below strike and the investor would receive physical delivery (shares), not principal.

**V1.0.1 corrected text:**
> - If at maturity ASML finishes at 65%: **below strike (100%)**, so **physical delivery applies**. Shares delivered: €500,000 / €700 = 714 ASML shares. Worth 714 × (0.65 × €700) = **€325,143**. Loss: **€174,857** (partially offset by coupons received).

**Verification:**
- €500,000 / €700 = 714.29 → 714 shares (standard rounding down)
- 714 × (0.65 × €700) = 714 × €455 = €324,870

Adjusted to exact: €500,000 / €700 × (0.65 × €700) = €500,000 × 0.65 = €325,000. Using 714 shares: 714 × €455 = €324,870. Loss: €175,130 (before coupons).

**Note:** The physical delivery formula on line 22566 (for the 55% scenario) is arithmetically correct and requires no change.

**Also recommended (R-04):** Add "Strike: 100% of initial" to the terms table at lines 22536-22545.
...
**Error:** Long single-stock vol + short basket vol is a **dispersion trade**, which profits when stocks diverge (low correlation). This is **short** correlation, not long.

The Desk Bible itself correctly describes the opposite trade at line 22483: "buying single-stock volatility and selling basket volatility" as the dispersion hedge. A dispersion trade is short correlation by construction.

**V1.0.1 corrected text (Option A — fix the label):**
...
**V1.0.1 corrected text (Option B — fix the legs to match the label):**
> | 6 | P3 | Dispersion hedging diagram | Flow | Bank's hedge: **long basket vol + short single-stock vol** = long correlation (reverse dispersion). Shows how the bank manages worst-of risk |

**Recommended:** Option A. The bank's described hedge at line 22483 is "buying single-stock volatility and selling basket volatility" — which matches the legs in the current text. The label should be "short correlation."
...
**Error:** The intermediate expression "3 × 2 = 6" yields −50 + 6 = −44, not −56. The stock dropped $2, so ΔS = −2. The correct intermediate step is 3 × (−2) = −6, giving −50 + (−6) = −56.

The final answer (−56) is correct. The error is in the intermediate expression, which drops the negative sign on the stock move.

**V1.0.1 corrected text:**
> - **Gamma effect:** After the $2 drop, your Delta changed. New Delta ≈ -50 + (3 × (−2)) = -50 − 6 = **-56**. Your exposure to further drops has increased.

**Rationale:** Fixes the intermediate arithmetic so a student following step-by-step gets the correct result. The economic intuition and final answer are unchanged.
...
**Total scope:** 10 line-level corrections across 3 artifacts. No structural changes.

---

*V1.0.1 Errata Addendum | FINAL | 2026-06-27*
