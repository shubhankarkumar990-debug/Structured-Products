# Interview System V2.0 — The Complete Structured Products Interview Handbook
...
DV01: The sensitivity to a 1bp rate move. A 30-year IRS has DV01 ≈ 25bp per $1M notional — a 100bp rate move creates ~$2.5M MTM exposure despite no principal exchange. (Trap: "No principal at risk means no risk" — FALSE, see Trap T.9.)
...
**Tier 2:** Construction: DRC + long down-and-out put. The KO barrier creates a discontinuity — the investor is exposed above the KO level but protected below it.
...
### Airbag Certificate — Complexity 5
...
### Booster Certificate — Complexity 6
**Tier 1:** Leveraged participation below a cap. 200% participation up to cap, 100% above. Leverage comes from selling upside above cap.
...
**CDS (covered in Top 10).** CommSwap (Commodity Swap) — Complexity 3: Exchanges fixed commodity price for floating market price. Basis risk (hedge vs actual commodity grade/location) is the key risk. §6.1: commodity day count conventions.
...
**Bonus Certificate — Complexity 5:** Guaranteed minimum return (bonus level) unless a barrier is breached during the observation period. If barrier is never breached: investor receives at least the bonus return. If breached: bonus is lost, investor receives underlying performance. Flash crash risk: a 5-second intraday barrier touch destroys the bonus.

**ICN (Issuer Callable Note) — Complexity 5:** A note where the bank can call at par on predetermined dates. The investor receives a higher coupon in exchange for the call risk. The call benefits the BANK, not the investor. §6.9: lifecycle event processing for issuer call.
...
**CLIQ (Cliquet / Ratchet) — Complexity 5:** Periodic returns are individually capped and floored, then summed. Local caps make the product underperform in trending markets but protect in volatile markets. The ratchet "locks in" periodic gains. §6.2: termsheet field — local cap, local floor, global floor.
...
**NCRA (Non-Callable Range Accrual) — Complexity 5:** Rate-linked range accrual — coupon accrues only when a reference rate stays within a range. If rates leave the range permanently in year 1 of a 10-year note, the investor receives zero coupon for the remaining 9 years. §6.1: rate fixing conventions, §6.6: P&L attribution for range accrual books.

**CRA SRT (Callable Range Accrual SRT) — Complexity 6:** NCRA plus bank call right. Two risks sold simultaneously: range risk (coupon depends on rates staying in range) + call risk (bank terminates when conditions favor the investor). §6.8: model risk — requires rate model calibration for both range and call.
...
**RA STEG (Range Accrual Steepener) — Complexity 6:** STEG with range accrual — coupon accrues only when CMS spread AND a rate condition are simultaneously met. Two conditions: (1) curve steepness, (2) rate level. Requires BOTH to earn coupon.
...
**Tier 3:** Worked example — the $55,556 difference: $100M notional, 6% annual rate, semi-annual coupon, accrual period January 15 to July 15 (181 actual days).
...
| 7 | Phoenix vs WOAC | WOAC adds worst-of basket → short correlation. Coupon doubles but risk is N stocks. |
...
2. Calculate the coupon payment amount. (Answer: EUR 1,000,000 × 8% × (180/360) = EUR 40,000. Note: 30/360 assumes 180 days for exactly 6 months.)
...
5. The stock closes at CHF 66.00 on maturity. What does the investor receive? (Answer: Physical delivery. Number of shares = EUR 1,000,000 / CHF 95.50 ≈ 10,471 shares. Investor receives ~10,471 depreciated shares worth CHF 66.00 each ≈ CHF 691,099 plus the EUR 40,000 coupon.)
...
IF.1 **Day Count:** $50M notional, 4.5% rate, 91-day period. Calculate accrued interest under (a) ACT/360, (b) ACT/365, (c) 30/360. (Answer: (a) $568,750, (b) $561,644, (c) $562,500. Spread between highest and lowest: $7,106.)
...
IF.8 **Bid-Offer Reserve:** Book of 50 RCs on illiquid mid-cap stocks, EUR 1M notional each, average bid-offer spread 3% of embedded option mid-market value. Calculate total bid-offer reserve. (Answer: 50 × EUR 1M × 1.5% (half-spread) = **EUR 750,000**.)
...
IF.15 **PPN Participation:** 5Y ZCB costs $92 (8% rate environment). Desk margin $1. ATM call costs $15. Participation rate? (Answer: Residual = $100 − $92 − $1 = $7. Participation = $7 / $15 = **46.7%.** In a 2% rate environment: ZCB costs ~$98, residual = $1, participation = $1/$15 = **6.7%.** Rate environment dominates PPN economics.)
...
## 9.1 Technical Traps (T.1–T.20)

**T.1 — "Delta is the probability of finishing ITM"**
...
**T.2 — "Capital-protected means risk-free"**
...
**T.3 — "Higher coupon = better product"**
...
**T.4 — "Diversification protects WOAC investors"**
...
**T.5 — "VarSwap short = limited risk like selling insurance"**
...
**T.6 — "Autocall is good for the investor"**
...
**T.7 — "Guaranteed coupon means safe product"**
...
**T.9 — "No principal at risk = no risk (IRS)"**
*Naive:* No principal exchange = safe. *Wrong because:* 30Y IRS DV01 ≈ 25bp/$1M. 100bp rate move = ~$2.5M MTM exposure. Enormous risk despite no principal.

**T.10 — "Low correlation = safer FTD"**
...
**T.11 — "Recovery rate doesn't matter for CDS"**
...
**T.12 — "European and American barrier are the same"**
...
**T.13 — "Vol smile is a market anomaly"**
...
**T.14 — "Monte Carlo is always the best pricing method"**
...
**T.15 — "Correlation is constant and observable"**
...
**T.16 — "PPNs are unattractive when rates are low"**
...
**T.17 — "Forward price = expected future spot"**
...
**T.18 — "Short-selling CDS is free money in good times"**
...
**T.19 — "Digital options are easy to hedge because payoff is simple"**
...
**T.20 — "Gaussian copula is 'wrong' for CDO pricing"**
...
Tests: Communication ability. Good analogy: "5 friends going to different parties — higher chance at least one gets sick."
...
**MC.14** (Advanced) VarSwap strike 20 vol (400 var). Realized 625 (25 vol). P&L per vega notional ≈: (a) 5 (b) 225/40 **(c) 5.625** (d) 12.5
...
## 10.3 Mini Case Assessment (8 Cases)

**Instructions:** For each: (1) recommended product or action, (2) reasoning, (3) one rejected alternative, (4) key risk. 90 minutes.

**MC.1** 65-year-old retiree, €300K, wants growth with no principal risk. Deposit rates 0.5%. Needs money in 5 years.
**MC.2** Corporate treasurer, €10M USD receivables over 6 months. Reports in EUR. Budget certainty priority.
**MC.3** Wealth manager client wants quarterly income >8%, some capital risk tolerance. PB cap 8. Single-stock preference.
**MC.4** Insurance company, 15-year liability, rate-linked asset generating 4%+. Comfortable with steepening risk, NOT call risk.
**MC.5** Hedge fund wants to express view that credit correlation will decrease. $50M risk budget. Comfortable with portfolio credit.
**MC.6** (NEW) You are a PC analyst. The desk's CDO book shows $200K of consistently positive unexplained P&L over 15 days. Each day passes the threshold individually. What is your recommended action?
**MC.7** (NEW) A CLN referencing a European bank is documented with MMR restructuring clause. The bank announces a voluntary subordinated debt extension. Does this trigger a credit event? What if the clause were NR?
**MC.8** (NEW) A 2-for-1 stock split is announced for the underlying of an RC with initial spot $100, strike $100, barrier $70. What are the post-split terms? What systems need updating?
...
| Role | Product Questions | Infrastructure Questions | Cases | Mock Track |
|------|------------------|------------------------|-------|------------|
...
| **Product Control** | IC.23-31, DS.23-28 | IC.23-31, IF.7-10 | D.9, D.14 | §8.7 |
| **Operations** | IC.44-50, DS.29-34 | IC.1-6, IC.44-50 | D.11, D.13 | §8.8 |
| **Treasury** | IC.32-38, DS.35-37 | IC.32-38, IF.9-14 | D.12 | §8.9 |
| **Finance** | IC.23-25 | IC.23-31 | D.14 | §8.10 |
| **Legal / Compliance** | IC.11-16, RG.1-30 | IC.11-22 | D.10 | §8.10 |
| **Market Risk** | T.1-20, DS.15-18, RG.23-30 | IC.17-22, IC.39-43 | D.5 | §8.5 variant |
| **Credit Risk** | T.27-32, IC.17-22 | IC.17-22 | D.10 | §8.5 variant |
